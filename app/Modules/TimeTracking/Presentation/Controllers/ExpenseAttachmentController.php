...
class ExpenseAttachmentController extends Controller
{
    ...
    public function store(Request $request, Expense $expense): JsonResponse
    {
        ...
        if (! in_array($mimeType, self::ALLOWED_MIME_TYPES, true)) {
            return response()->json(['message' => __('time_tracking.attachment_file_type_not_allowed')], 422);
        }

        if ($file->getSize() > self::MAX_FILE_SIZE_BYTES) {
            return response()->json(['message' => __('time_tracking.attachment_too_large')], 422);
        }
        ...
        if ($path === false) {
            return response()->json(['message' => __('time_tracking.attachment_save_failed')], 500);
        }
    ...
    public function show(Expense $expense): StreamedResponse|JsonResponse
    {
        ...
        if (! $expense->hasAttachment()) {
            return response()->json(['message' => __('time_tracking.attachment_missing')], 404);
        }
