@php
    /** @var \App\Modules\Invoicing\Application\DTOs\InvoicePdfViewModel $vm */
    $invoice  = $vm->invoice;
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .doc-sub { font-size: 9px; color: #6b7280; text-align: right; }
        ...
        .work-report-link { text-decoration: underline; color: #1a1a1a; }
        ...
        .czk-box { background: #f2f2f2; padding: 8px; margin-top: 10px; }
        ...
        .qr-label { font-size: 8px; color: #6b7280; text-transform: uppercase; letter-spacing: 0.05em; text-align: right; }
        ...
        /* Work report page */
        .work-report-page { page-break-before: always; }
        .wr-title { font-size: 15px; font-weight: bold; margin-bottom: 4px; }
        .wr-sub { font-size: 9px; color: #6b7280; margin-bottom: 16px; }
        .wr-total td { font-weight: bold; border-top: 2px solid #111; }
    </style>
</head>
<body>
<div class="page">
    ...
    <table class="header">
        <tr>
            ...
            <td style="width:45%">
                ...
                @endif
                @if($vm->relatedInvoiceNumber)
                    <div class="doc-sub">{{ __('invoices::pdf.related_invoice') }} {{ $vm->relatedInvoiceNumber }}</div>
                @endif
            </td>
        </tr>
    </table>
    ...
    <table class="meta">
        <tr>
            <td style="width:55%">
                <table>
                    @if($vm->bank !== null)
                        @if(!empty($vm->bank['bank_name']))
                            <tr><td class="meta-label">{{ __('invoices::pdf.bank') }}</td><td class="meta-value">{{ $vm->bank['bank_name'] }}</td></tr>
                        @endif
                        @if(!empty($vm->bank['account_number']))
                            <tr><td class="meta-label">{{ __('invoices::pdf.account_number') }}</td><td class="meta-value">{{ $vm->bank['account_number'] }}</td></tr>
                        @endif
                        @if(!empty($vm->bank['iban']))
                            <tr><td class="meta-label">{{ __('invoices::pdf.iban') }}</td><td class="meta-value">{{ $vm->bank['iban'] }}</td></tr>
                        @endif
                        @if(!empty($vm->bank['bic']))
                            <tr><td class="meta-label">{{ __('invoices::pdf.bic') }}</td><td class="meta-value">{{ $vm->bank['bic'] }}</td></tr>
                        @endif
                    @endif
                    @if($invoice->variable_symbol)
                        <tr><td class="meta-label">{{ __('invoices::pdf.variable_symbol') }}</td><td class="meta-value">{{ $invoice->variable_symbol }}</td></tr>
                    @endif
                </table>
            </td>
            <td style="width:45%">
                <table>
                    ...
                    @endif
                    <tr>
                        <td class="meta-label">{{ __('invoices::pdf.due_at') }}</td>
                        <td class="meta-value right {{ $invoice->isOverdue() ? 'overdue' : '' }}">
                            {{ $invoice->due_at?->format('d.m.Y') }}@if($invoice->isOverdue()) ({{ __('invoices::pdf.overdue') }})@endif
                        </td>
                    </tr>
                </table>
            </td>
    ...
    @if($invoice->note_above)
        <div class="note">{{ $invoice->note_above }}</div>
    @endif
    ...
    {{-- Summary: VAT recap + CZK conversion left, totals + QR right --}}
    <table class="summary">
        <tr>
            <td style="width:52%">
                @if($vm->vatRecap !== [])
                ...
                @if($vm->czkRecap !== null)
                    <div class="czk-box">
                        ...
                        <table class="recap">
                            ...
                            @foreach($vm->czkRecap as $row)
                                <tr>
                                    <td>{{ number_format($row->rate, 0) }}%</td>
                                    <td>{{ $fmt($row->base) }}</td>
                                    <td>{{ $fmt($row->vat) }}</td>
                                    <td>{{ $fmt($row->total) }} Kč</td>
                                </tr>
                            @endforeach
                        </table>
                    </div>
            ...
            <td style="width:40%">
                <table class="totals">
                    <tr>
                        ...
                        <td class="right">{{ $fmt($invoice->subtotal) }} {{ $currency->symbol() }}</td>
                    </tr>
                    @if((float) $invoice->discount_amount > 0)
                        <tr>
                            <td>{{ __('invoices::pdf.discount') }} @if($invoice->discount_percent)({{ number_format((float) $invoice->discount_percent, 0) }}%)@endif</td>
                ...
                @if($vm->qrDataUri)
                    <div class="qr">
                        <img src="{{ $vm->qrDataUri }}" alt="QR">
                        <div class="qr-label">{{ __('invoices::pdf.qr_payment') }}</div>
                    </div>
                @endif
            </td>
        </tr>
    ...
    @if($vm->hasWorkReport())
        <div class="work-report-page">
            ...
            <div class="wr-title">{{ __('invoices::pdf.work_report') }}</div>
            <div class="wr-sub">{{ $vm->documentTitle }}</div>

            <table class="items">
                <thead>
                <tr>
                    <th style="width:18%">{{ __('invoices::pdf.work_date') }}</th>
                    <th style="width:64%">{{ __('invoices::pdf.description') }}</th>
                    <th class="right" style="width:18%">{{ __('invoices::pdf.hours') }}</th>
                </tr>
                </thead>
                <tbody>
                @foreach($vm->workReportLines as $line)
                    <tr>
                        <td>{{ $line->work_date->format('d.m.Y') }}</td>
                        <td>{{ $line->description }}</td>
                        <td class="right">{{ $fmt($line->hours) }}</td>
                    </tr>
                @endforeach
                <tr class="wr-total">
                    <td colspan="2">{{ __('invoices::pdf.total_hours') }}</td>
                    <td class="right">{{ $fmt($vm->totalHours()) }}</td>
                </tr>
                </tbody>
            </table>
        </div>
