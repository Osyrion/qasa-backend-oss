...
| GET | `/api/v1/profile/setup-status` | onboarding checklist (`SetupStatusService`) | `auth:sanctum` |
...
**Setup checklist** (`SetupStatusController` → `SetupStatusService`, rovnaký režim ako
`GET /me` — číta len vlastné metadáta, žiadna nová ability) vracia sedem položiek
(`{key, done, optional}`) pre onboarding wizard vo frontende: `billing_identity` (IČO,
adresa, mesto, krajina), `vat_status` (`vat_status_confirmed_at IS NOT NULL` — samotný
stĺpec `vat_status` má DB default, takže nevie rozlíšiť „potvrdené" od „nikdy nevidené";
`UpdateProfileAction` timestamp nastaví zakaždým, keď request obsahuje `vat_status` alebo
`is_vat_payer`), `bank_account`, `first_client`, `first_invoice` (nepovinné, aj draft sa
počíta), `logo` (nepovinné), `invoice_numbering` (nepovinné — vlastný `invoice_number_mask`
alebo aspoň `invoice_prefix`, ktorý má vždy default `"FA"`, takže táto položka je z
podstaty splnená aj na čerstvom účte). `completed` = všetky **ne-nepovinné** položky
`done`. Pre SaaS tímového člena sa číta stav **účtu** (`accountOwner()`), nie prihláseného
používateľa. Sedem lacných `exists()`/null-check dopytov na request, žiadny cache — stav sa
má prepnúť okamžite po akcii, nie po TTL.
...
| Metóda | URI | Účel |
|---|---|---|
| — | `/events` (`apiResource`) | plný REST nad udalosťami; index prijíma `order_id` (filter) a `include=order_deadlines` |
...
**Prepojenie na zákazku** — `order_id` (nullable, FK `nullOnDelete`: zmazanie zákazky
udalosť len odviaže, nezmaže). Klient nemá vlastný stĺpec (dosiahnuteľný tranzitívne cez
`order->client`, aby nevznikli dva zdroje pravdy) — udalosť viazaná len na klienta bez
zákazky zatiaľ nie je podporovaný use-case (YAGNI). Cudzí `order_id` pri vytvorení/úprave
rieši globálny `HasUserScope` scope na `Order::findOrFail()` → prirodzené `404` (rovnaký
idiom ako pri `TimeEntry`/zákazke). `EventResource` vracia `order_id` aj vnorený `order:
{id, name, color, client_display_name}` (eager-load `order.client`, žiadne N+1) a
`effective_color` — vlastná farba udalosti, inak farba zákazky (čisto prezentačný fallback,
stĺpec `color` sa nemení).

**Deadliny zákaziek v kalendári** (`GET /events?include=order_deadlines`) — primieša do
`data` **virtuálne** celodenné položky (`type: "order_deadline"`, `id: null`, needitovateľné)
za zákazky účtu so stavom `active`/`paused` a `deadline` v dopytovanom rozsahu (`from`/`to`).
Nič sa nepersistuje — položky sa počítajú nanovo pri každom requeste zo živých dát
zákaziek, takže zmena deadlinu je vidieť okamžite bez synchronizácie. Export (ICS/CSV) ich
nikdy nezahŕňa — exportujú sa len reálne udalosti, aby re-import nevytváral duplicity.
...
- **`Invoice`**: centrálny doklad. `user_id, client_id, invoice_number` (nullable — `null`
  pre draft, pridelí sa až pri vystavení, pozri nižšie), `type`
...
**Číslo sa prideľuje až pri vystavení, nie pri vytvorení draftu** (`IssueInvoiceAction`,
jediné miesto volania `nextInvoiceNumber()`). Draft má `invoice_number = null` a
`variable_symbol = null`; PDF náhľad/stiahnutie draftu vykreslí namiesto čísla lokalizovaný
placeholder (`invoices::pdf.draft_number_placeholder`, "KONCEPT"/"DRAFT"). Dôvod: draft sa
dá zmazať (`InvoicePolicy::delete`) a dlho ležiaci draft by inak dostal číslo „svojej doby"
namiesto poradia podľa skutočného vystavenia — SK/CZ účtovná prax vyžaduje súvislý,
chronologický rad bez dier. Existujúce (pred touto zmenou vytvorené) drafty s už prideleným
číslom si ho ponechávajú — `IssueInvoiceAction` číslo prideľuje len keď je `NULL`, nikdy ho
neprepisuje.
...
- **`CreateInvoiceAction`** — vytvorí draft bez čísla (`invoice_number = null`), prevezme
  variabilný symbol z requestu ak prišiel (inak `null` až do vystavenia), predvolí bankový
  účet podľa meny.
- **`IssueInvoiceAction`** — okamih **draft → vystavená/vydaná**: ak doklad ešte nemá číslo,
  pridelí ďalšie poradové podľa typu/prefixu (`nextInvoiceNumber()` — jediné volanie v celej
  fakturácii) a odvodí z neho variabilný symbol; zmrazí snapshoty dodávateľa, klienta a
  bankového účtu (neskoršia úprava profilu už doklad spätne nezmení); pre cudziu menu zmrazí
  kurz ČNB (zlyhanie nie je fatálne — PDF len vynechá CZK tabuľku).
...
4. **Digest vlastníkovi** (`users.overdue_digest_enabled`, predvolene **zapnuté** —
   nezávisle od `auto_remind_enabled`, ide o notifikáciu vlastníkovi, nie klientovi) — faktúry,
   ktorým v tomto behu (kroku 1) práve pribudol marker `overdue_notified_at`, sa per
   používateľ zozbierajú a po skončení jeho slučky pošlú v **jednom** e-maile
   `OverdueInvoicesDigestMail` (číslo, klient zo snapshotu, suma+mena, dní po splatnosti).
   Nikdy nie jeden e-mail na faktúru — výpadok cronu alebo návrat z dovolenky by inak
   používateľa zaplavil. Idempotencia zadarmo cez existujúci marker: druhý beh tie isté
   faktúry do digestu nezaradí.

Zlyhanie na jednej faktúre (napr. chýbajúci e-mail klienta) alebo pri odoslaní digestu sa
loguje a beh pokračuje.
